...
class FixMatch:
    def __init__(self, net_builder, num_classes, ema_m, T, p_cutoff, lambda_u,
        ...
        self.target_counts_ratio = np.zeros(self.num_classes)
    ...
    def train(self, args, logger=None):
        ...
        amp_cm = autocast if args.amp else nullcontext

        #         target_counts_ratio_next = np.ones([100, self.num_classes]) / self.num_classes

        if self.auxiliary:
            auxiliary_iter = self.loader_dict['auxiliary_ulb'].__iter__()

        for i, ((x_lb, y_lb), (x_ulb_w, x_ulb_s, y_pesudo_ulb)) in enumerate(zip(self.loader_dict['train_lb'],
            ...
            x_lb, x_ulb_w, x_ulb_s = x_lb.cuda(args.gpu, non_blocking=True), x_ulb_w.cuda(args.gpu, non_blocking=True), x_ulb_s.cuda(args.gpu, non_blocking=True)
            y_lb = y_lb.cuda(args.gpu, non_blocking=True)
            y_pesudo_ulb = y_pesudo_ulb.cuda(args.gpu, non_blocking=True)

            if not self.auxiliary:
                inputs = torch.cat((x_lb, x_ulb_w, x_ulb_s))
            else:
                x_au_w, x_au_s, _ = auxiliary_iter.__next__()
                x_au_w, x_au_s = x_au_w.cuda(args.gpu, non_blocking=True), x_au_s.cuda(args.gpu, non_blocking=True)
                inputs = torch.cat((x_lb, x_ulb_w, x_ulb_s, x_au_w, x_au_s))
            # inference and calculate sup/unsup losses
            with amp_cm():
                logits = self.train_model(inputs)
                logits_x_lb = logits[:num_lb]
                if self.auxiliary:
                    logits_x_ulb_w, logits_x_ulb_s, logits_au_w, logits_au_s = logits[num_lb:].chunk(4)
                else:
                ...
                prob_lb = F.softmax(logits_x_lb, dim=1)
                prob_lb_cat = torch.gather(prob_lb, 1, torch.reshape(y_lb, [-1, 1])).squeeze()

                prob_lb_median = torch.zeros(self.num_classes).cuda(args.gpu, non_blocking=True)
                for i in range(self.num_classes):
                    prob_lb_median[i] = (y_lb == i).any() and prob_lb[:, i][y_lb == i].median() or 0
                clean_refine = prob_lb_cat >= prob_lb_median[y_lb]
                refine_labeled = clean_refine.float().mean().detach()
                sup_loss = ce_loss(logits_x_lb[clean_refine], y_lb[clean_refine], reduction='mean')
                ...
                if self.auxiliary:
                    au_loss = consistency_loss(logits_au_w,
                                               logits_au_s,
                                               None,
                                               name='bce')
                    total_loss = total_loss + 0.1 * au_loss
            ...
            tb_dict['train/refine_labeled'] = refine_labeled
