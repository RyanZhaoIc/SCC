...
class STL10(Dataset):
    ...
    def __init__(self, root=MyPath.db_root_dir(),
        ...
        #         self.split = split
        #         assert self.split in ['train', 'train+unlabeled', 'unlabeled', 'test', 'train+test']
        ...
        if self.split == 'train':
            ...
            test_data, test_labels = self.__loadfile(
                self.test_list[0][0], self.test_list[1][0])
            unlabeled_data, _ = self.__loadfile(self.train_list[2][0])
            self.data = np.concatenate((self.data, test_data, unlabeled_data))
            self.labels = np.concatenate(
                (self.labels, test_labels, np.asarray([-1] * unlabeled_data.shape[0])))
