...
def get_criterion(p):
    ...
    elif p['criterion'] == 'cross-entropy':
...
def get_model(p, pretrain_path=None):
    ...
    if p['backbone'] == 'resnet18':
        ...
        if p['train_db_name'] == 'stl-10':
            from ..models.wrn_stl10 import wrn_stl
            backbone = wrn_stl()
        else:
            raise NotImplementedError
    ...
    elif p['setup'] in ['scan', 'selflabel', 'reliability']:
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
                      neighbor_path=None,
                      split=None, indices=None):
    # Base dataset
    if p['train_db_name'] == 'cifar-10':
        from ..data.cifar import CIFAR10
        dataset = CIFAR10(split='train+test', transform=transform, download=True, indices=indices)

    elif p['train_db_name'] == 'cifar-20':
        from ..data.cifar import CIFAR20
        dataset = CIFAR20(split='train+test', transform=transform, download=True, indices=indices)

    elif p['train_db_name'] == 'stl-10':
        from ..data.stl import STL10
        dataset = STL10(split=split, transform=transform, download=True, indices=indices)
    ...
    if neighbor_path is not None:  # Dataset returns an image and one of its nearest neighbors.
        from ..data.custom_dataset import NeighborsDataset
        neighbors = np.load(neighbor_path)
        dataset = NeighborsDataset(dataset, neighbors, p['num_neighbors'])
...
def get_optimizer(p, model, cluster_head_only=False):
    if cluster_head_only:  # Only weights in the cluster head will be updated
        for name, param in model.named_parameters():
            if ('cluster_head' in name) or ('transition' in name):
...
def adjust_learning_rate(p, optimizer, epoch):
    ...
    elif p['scheduler'] == 'fix_match':
        lr = lr * math.cos((7 * math.pi * epoch) / (16 * p['epochs']))
