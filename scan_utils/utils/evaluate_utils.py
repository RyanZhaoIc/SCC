...
def hungarian_evaluate(subhead_index, all_predictions, class_names=None,
    ...
    targets = head['targets'][head['targets'] >= 0].cuda()
    predictions = head['predictions'][head['targets'] >= 0].cuda()
    probs = head['probabilities'][head['targets'] >= 0].cuda()
