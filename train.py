...
from fix_utils.utils import net_builder, get_logger, count_parameters, ssl_path
from fix_utils.train_utils import TBLog, get_SGD, get_cosine_schedule_with_warmup
from models.fixmatch.fixmatch import FixMatch
from datasets.ssl_dataset import SSL_Dataset
from datasets.data_utils import get_data_loader


# print(np.transpose(estimate_dataset.data, (1, 0, 2, 3)).reshape([3, -1]).mean(1) / 255)
# print(np.transpose(estimate_dataset.data, (1, 0, 2, 3)).reshape([3, -1]).std(1) / 255)
...
def main(args):
    ...
    args.save_dir = ssl_path(args.save_dir, args.dataset)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dset = SSL_Dataset(name=args.dataset, train=True,
                             num_classes=args.num_classes, data_dir=args.data_dir, save_dir=args.save_dir)
    ...
    if args.auxiliary:
        _auxiliary_dset = SSL_Dataset(name=args.dataset, train=True,
                                      num_classes=args.num_classes, data_dir=args.data_dir, save_dir=args.save_dir,
                                      auxiliary=True)
        _, auxiliary_dset = _auxiliary_dset.get_ssl_dset(args.num_labels)

        dset_dict = {'train_lb': lb_dset, 'auxiliary_ulb': auxiliary_dset, 'train_ulb': ulb_dset, 'eval': eval_dset}

        loader_dict['auxiliary_ulb'] = get_data_loader(dset_dict['auxiliary_ulb'],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--auxiliary', action='store_true',
                        help='use auxiliary overclustering')
